...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CLAUDE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CLAUDE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Claude API.
 */
export class Claude {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['CLAUDE_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CLAUDE_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ClaudeError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Claude client with an apiKey option, like new Claude({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Claude.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CLAUDE_LOG'), "process.env['CLAUDE_LOG']", this) ??
      defaultLogLevel;
  ...
  static ClaudeError = Errors.ClaudeError;
